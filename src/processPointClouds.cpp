...
ProcessPointClouds<PointT>::FilterCloud(typename pcl::PointCloud<PointT>::Ptr cloud, float filterRes,
    ...
    // Downsample to functional requirement size and create Object
...
ProcessPointClouds<PointT>::RansacPlane(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations,
    ...
    while (maxIterations--) {
        ...
        float a, b, c, d, x1, x2, x3, p4, y1, y2, y3, y4, z1, z2, z3, z4;
...
template<typename PointT>
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr>
ProcessPointClouds<PointT>::SegmentPlaneUsingRansac3D(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations,
                                                      float distThresh) {
    // Time segmentation process
    auto startTime = std::chrono::steady_clock::now();
    // Done: Fill in this function to find inliers for the cloud.
    pcl::PointIndices::Ptr inliers{new pcl::PointIndices}; // Init Shared Ptr
    std::unordered_set<int> inlierIndices = RansacPlane(cloud, maxIterations, distThresh);
...
template<typename PointT>
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::ClusteringEuclideanUsingKDtree(
    ...
    tree->setInputCloud(cloud); // replace with K
    std::vector<pcl::PointIndices> clusterIndices;

    auto res = euclideanCluster(cloud, tree, clusterTolerance, minSize, maxSize);

    for (auto clusterIs: res) {
        pcl::PointIndices indices;
        indices.indices = clusterIs;
